"""
Forwarder shim to the archived monolith.
This preserves the original import path used by tests and scripts after archiving.
"""
...
def _load_archived_monolith():
    here = Path(__file__).resolve().parent
    candidates = [
        here.parent / 'archive' / '01_generate_game_states_enhanced.py',
        here / 'archive' / '01_generate_game_states_enhanced.py',
    ]
    for path in candidates:
        if path.exists():
            ...
            return mod
    raise ImportError('Archived monolith not found. Expected in ./archive/.')


_monolith = _load_archived_monolith()

# Re-export public attributes for backward compatibility
__all__ = [name for name in dir(_monolith) if not name.startswith('_')]
globals().update({name: getattr(_monolith, name) for name in __all__})


def main(argv=None):
    # Delegate CLI to archived module if it exposes a main
    if hasattr(_monolith, 'main'):
        return _monolith.main(argv)
    # Fallback: run the generator with default args if pandas is available
    if hasattr(_monolith, 'generate_tic_tac_toe_dataset'):
        return _monolith.generate_tic_tac_toe_dataset()
    raise SystemExit(0)
#!/usr/bin/env python3
"""
Enhanced Tic-Tac-Toe game state generator with exact game-theoretic solution.
Creates comprehensive ML-friendly datasets with perfect-play labels, tactical features,
policy targets, and symmetry-aware canonicalization/augmentation.
"""
...
try:
    ...
    pd = None  # Pandas optional for import; required for dataset export

logger = logging.getLogger(__name__)

# --- Legacy compatibility re-exports (non-invasive) ---
# These help tests and existing imports continue to work while we refactor into modules.
try:
    # Lightweight imports; if refactor modules exist, use them; else fall back to local defs.
    from ttt.game_basics import (
        serialize_board as _gb_serialize_board,
        deserialize_board as _gb_deserialize_board,
        get_winner as _gb_get_winner,
        is_draw as _gb_is_draw,
    )
    serialize_board = _gb_serialize_board
    deserialize_board = _gb_deserialize_board
    get_winner = _gb_get_winner
    is_draw = _gb_is_draw
except Exception:
    pass
try:
    from ttt.symmetry import (
        transform_board as _sym_transform_board,
        apply_action_transform as _sym_apply_action_transform,
        symmetry_info as _symmetry_info,
        ALL_SYMS as _ALL_SYMS,
    )
    transform_board = _sym_transform_board
    apply_action_transform = _sym_apply_action_transform
    symmetry_info = _symmetry_info
    ALL_SYMS = _ALL_SYMS
except Exception:
    pass
try:
    from ttt.solver import solve_state as _sl_solve_state
    solve_state = _sl_solve_state
except Exception:
    pass
try:
    from ttt.policy_targets import (
        build_policy_targets as _pt_build_policy_targets,
        epsilon_policy_distribution as _pt_epsilon_policy_distribution,
        compute_action_ranks as _pt_compute_action_ranks,
        difficulty_score as _pt_difficulty_score,
    )
    build_policy_targets = _pt_build_policy_targets
    epsilon_policy_distribution = _pt_epsilon_policy_distribution
    compute_action_ranks = _pt_compute_action_ranks
    difficulty_score = _pt_difficulty_score
except Exception:
    pass


# ========== Global Tic-Tac-Toe pattern constants ==========
# Single source of truth for all win line patterns, used throughout the module.
WIN_PATTERNS = [
...
def serialize_board(board: List[int]) -> str:
    """Convert board list to string representation."""
    return ''.join(str(cell) for cell in board)


def deserialize_board(board_str: str) -> List[int]:
    """Convert string representation to board list."""
...
def get_winner(board: List[int]) -> int:
    ...
    Returns: 1 for X win, 2 for O win, 0 for no winner
    """
...
def get_winning_line(board: List[int]) -> List[int]:
    """Return the indices of the winning line, or empty list if no winner."""
    for pattern in WIN_PATTERNS:
        values = [board[i] for i in pattern]
        if values[0] != 0 and values[0] == values[1] == values[2]:
            return pattern
...
def is_valid_state(board: List[int]) -> bool:
    ...
    # X goes first, so X count should be equal to O or one more
    if not (x_count == o_count or x_count == o_count + 1):
        return False
    
    winner = get_winner(board)
    
    # If X won, X should have made the last move
    if winner == 1 and x_count != o_count + 1:
        return False
    
    # If O won, O should have made the last move
    if winner == 2 and x_count != o_count:
        return False
    
    # Check for multiple winners (impossible in valid game)
    x_wins = count_winning_lines(board, 1)
    o_wins = count_winning_lines(board, 2)
