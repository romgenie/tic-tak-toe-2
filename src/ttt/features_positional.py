"""
Positional features and basic patterns:
- Line threats (row/col/diag)
- Connectivity and clusters
- Control metrics (center/corners/edges)
- Pattern strengths and per-cell open lines
    ...
    for i in range(9):
        if board[i] != 2:
            ...
            for pat in WIN_PATTERNS:
                if i in pat and not any(board[j] == 2 for j in pat):
                    cnt += 1
            x_pot[i] = cnt
        if board[i] != 1:
            cnt = 0
            for pat in WIN_PATTERNS:
                if i in pat and not any(board[j] == 1 for j in pat):
                    cnt += 1
            o_pot[i] = cnt
