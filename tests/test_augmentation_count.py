...
def test_augmentation_emits_more_rows_per_state():
    ...
    rows_no_aug = generate_state_action_dataset(
        subset, include_augmentation=False, canonical_only=False
    )
    rows_aug = generate_state_action_dataset(
        subset, include_augmentation=True, canonical_only=False
    )
