...
def main(argv=None):  # delegate CLI behavior if present
    ...
    if hasattr(_mod, "generate_tic_tac_toe_dataset"):
        ...
        parser = argparse.ArgumentParser(
            description="Generate Tic-Tac-Toe datasets (archived monolith)"
        )
...
def action_tactical_labels(
    board: List[int], move: int, sol: Optional[Dict] = None
) -> Dict[str, int]:
    ...
    labels["is_safe_move"] = int(
        labels["legal"]
        and (not labels["gives_opponent_immediate_win"])
        and (blocks_opponent_fork(board, p, move) or len(fork_moves(board, opp)) == 0)
    )
    ...
    if sol is not None and sol["q_values"][move] is not None:
        labels["creates_trap_in_2"] = int(
            sol["q_values"][move] == +1 and sol["dtt_action"][move] == 2
        )
    else:
...
def action_robustness_metrics(
    board: List[int], move: int, solved_map: Dict[str, dict]
...
def calculate_line_threats(board: List[int], player: int) -> Dict[str, int]:
    ...
    threats = {"row_threats": 0, "col_threats": 0, "diag_threats": 0, "total_threats": 0}
...
def extract_board_features(
    ...
    opponent_immediate_wins = count_immediate_wins(
        normalized_board, 2 if current_player == 1 else 1
    )
    ...
    features = {
        ...
        "plies_to_win_current": (
            (plies_to_end if (value_current == +1) else None) if reachable else None
        ),
        "plies_to_lose_current": (
            (plies_to_end if (value_current == -1) else None) if reachable else None
        ),
...
def generate_state_action_dataset(
    ...
    for key, sol in solved_map.items():
        ...
        eps_pols = {
            f"{int(round(e*100)):03d}": epsilon_policy_distribution(b, sol, e) for e in epsilons
        }
        ...
        for i in range(9):
            ...
            advantage = (
                None if sol["q_values"][i] is None else float(sol["q_values"][i] - sol["value"])
            )
        ...
        if include_augmentation:
            ...
            for sym_op in ALL_SYMS[1:]:  # Skip identity
                ...
                transformed_optimal = {
                    apply_action_transform(m, sym_op) for m in sol["optimal_moves"]
                }
                ...
                for i in range(9):
                    ...
                    next_dtt_t = (
                        solved_map[child_key]["plies_to_end"] if child_key in solved_map else None
                    )
                    advantage_t = (
                        None
                        if transformed_q[i] is None
                        else float(
                            transformed_q[i]
                            - (
                                solved_map[transformed_key]["value"]
                                if transformed_key in solved_map
                                else 0
                            )
                        )
                    )
                    ...
                    rows.append(
                        {
                            ...
                            **{
                                f"epsilon_policy_{tag}": transformed_eps_pols[tag][i]
                                for tag in transformed_eps_pols
                            },
                            ...
                            "gives_opponent_immediate_win": orig_data[
                            ...
                            "orbit_int_id": orbit_int_id(
                                sym_t["canonical_form"], canonical_action_t
                            ),
                            "advantage": advantage_t,
                            "within_tier_pref": (
                                within_tier_sign(solved_map[transformed_key], i)
                                if transformed_key in solved_map
                                else None
                            ),
                            "within_tier_margin": (
                                within_tier_margin(solved_map[transformed_key], i)
                                if transformed_key in solved_map
                                else None
                            ),
                ...
                rows.append(
                    {
                        ...
                        "q_diff": q[a] - q[b],
                        "dtt_diff": (
                            (d[a] - d[b]) if (d[a] is not None and d[b] is not None) else None
                        ),
                    }
                )
    return rows
...
def reply_branching_factor_after_best_reply(
    board: List[int], sol: Dict, solved_map: Dict[str, dict]
) -> float:
...
def generate_tic_tac_toe_dataset(
    ...
    for state in itertools.product([0, 1, 2], repeat=9):
        ...
        feats = extract_board_features(
            board, solved_map, lambda_temp=lambda_temp, q_temp=q_temp, epsilons=epsilons
        )
    ...
    for _, row in df_states.iterrows():
        ...
        if cf not in canon_groups:
            canon_groups[cf] = (
                int(row["move_number"]),
                int(row["winner_perfect"]) if not pd.isna(row["winner_perfect"]) else 0,
            )
    ...
    if export_npz:
        ...
        def to_npz(df: Any, out_path: str):
            ...
            if (
                x_plane is not None
                and o_plane is not None
                and empty_plane is not None
                and to_move_plane is not None
            ):
            ...
            if planes is not None:
                np.savez_compressed(
                    out_path, data=X, columns=cols, planes=planes, split_index=split_idx
                )
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate Tic-Tac-Toe datasets with exact labels and rich features"
    )
    parser.add_argument(
        "--no-augmentation", action="store_true", help="Disable symmetry augmentation for actions"
    )
    parser.add_argument(
        "--no-canonical-only",
        action="store_true",
        help="Include all symmetry variants (not just canonical)",
    )
    parser.add_argument(
        "--all-states",
        action="store_true",
        help="Include unreachable/invalid states (override reachable/valid filters)",
    )
    ...
    parser.add_argument(
        "--lambda-temp", type=float, default=0.5, help="Lambda temperature for soft DTT policy"
    )
    parser.add_argument(
        "--q-temp", type=float, default=1.0, help="Temperature for Q softmax policy"
    )
    parser.add_argument(
        "--epsilons",
        type=str,
        default="0.1",
        help='Comma-separated epsilon values, e.g. "0.05,0.1,0.2"',
    )
    parser.add_argument(
        "--states-canonical-only", action="store_true", help="Deduplicate states by canonical form"
    )
    parser.add_argument(
        "--seed", type=int, default=42, help="Random seed for splitting and trajectories"
    )
    parser.add_argument(
